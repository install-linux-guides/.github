# Contributing to Install Linux Guides
[heading__title]:
  #contributing-to-install-linux-guides
...
The text within this document are not _set in stone_ rules but guidelines, and much like other files maintained by Install Linux Guides the following may be edited via a _`Pull Request`_.
...
Review the [whole thing][branch__current__code_of_conduct] to see what is expected of those that maintain code with Install Linux Guides
...
Join the <sub>[![Contributors][badge__contributors]][branch__current__network_members]</sub> issuing Pull Requests that close available <sub>[![Open Issues][badge__issues]][branch__current__issues]</sub>. [New Issues][branch__current__issues_new] may be opened for Reporting Bugs and Suggesting Enhancements. However, please search for existing similar Issues first; example search for [`memory-leaks`](https://github.com/install-linux-guides/.github/search?q=memory-leaks&type=Issues) from the `.github` repository.
...
There may be _`orphan`_ branches utilized by install-linux-guides for separating Code to be included in other projects from Documentation and Usage Examples. Non-orphaned development branches are encouraged when adding features or fixing bugs.
...
[branch__current__network_members]:
  network/members
  "Perhaps you too may be counted amount those that have Forked repositories maintain by Install Linux Guides"

[branch__current__support]:
  SUPPORT.md
  "Best avenues for seeking support of Install Linux Guides"
...
[badge__issues]: https://img.shields.io/github/issues/install-linux-guides/.github.svg

[badge__contributors]: https://img.shields.io/github/forks/install-linux-guides/.github.svg?color=005571&label=Contributors
